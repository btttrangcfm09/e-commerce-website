...
function getPublicLocalImageUrl(req, file) {
    // multer dest is uploads/users, file.filename is generated by multer
...
const uploadProfileImage = async (req, res, next) => {
    try {
        // Nếu request không phải multipart/form-data (ví dụ Save profile bằng JSON)
        // thì bỏ qua multer để tránh lỗi boundary + không overwrite req.body.image.
        ...
        upload.single('image')(req, res, async (err) => {
            ...
            // If Google Drive credentials are not configured, fall back to local upload.
            if (!hasGoogleDriveConfig()) {
                req.body.image = getPublicLocalImageUrl(req, req.file);
                return next();
            }
            ...
            try {
                ...
                // If Drive upload fails (bad credentials, etc.), keep local file so user can continue.
                req.body.image = getPublicLocalImageUrl(req, req.file);
                return next();
            }
        });
